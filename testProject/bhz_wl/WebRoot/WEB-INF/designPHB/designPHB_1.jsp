    	...
    	<form id="updateForm" action="designPHBAction!updateDesignPHB.action?bdCode=${bdCode}&bhzCode=${bhzCode}" method="post">
	        ...
	        <table class="message2" >
	            <tr class="onbg">
		            ...
		            <td colspan="${fn:length(paramList)}">各参数的设计配合比</td>
	            </tr>
	            <tr class="onbg">
	            	<c:choose>
				...
				<c:when test="${fn:length(designPhbs)>0}">
		            <c:forEach var="d" items="${designPhbs}" varStatus="i">
		            	<tr>
		            		<td>${map1[d.bdCode]}</td>
		            		<td>${map2[d.bhzCode]}</td>
		            		<td>${d.intensityGrade}</td>
			             	<c:forEach var="p" items="${paramList}">
			 					<td>
	            ...
	            <c:otherwise>
	            	<tr><td height="25" colspan="${fn:length(paramList) + 4}" style="text-align: center;color: red;">“${map1[bdCode]}”的“${map2[bhzCode]}”下还没有设置强度等级，请先去强度等级模块设置!</td></tr>
	            </c:otherwise>
	            </c:choose>
	            </c:when>
	            <c:otherwise>
	            <tr><td height="25" colspan="${fn:length(paramList) + 4}" style="text-align: center;color: red;">“${map1[bdCode]}”的“${map2[bhzCode]}”下还没有设置参数，请先去参数管理模块设置!</td></tr>
	            </c:otherwise>
	            </c:choose>   
	        </table>
        </form>
